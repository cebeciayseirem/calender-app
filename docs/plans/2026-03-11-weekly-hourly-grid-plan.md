# Weekly Hourly Grid Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Rewrite the weekly view to display events on a 24-hour CSS Grid with time labels, hour lines, and sticky day headers.

**Architecture:** Single file rewrite of `src/components/views/weekly-view.tsx`. Remove all @dnd-kit imports and drag logic. Replace with a CSS Grid layout (48px time gutter + 7 day columns, 24 hour rows). Events snap to their start hour's row. Auto-scroll to 7 AM on mount.

**Tech Stack:** React 19, TypeScript, Tailwind CSS v4, date-fns

---

### Task 1: Strip DnD and scaffold the grid shell

**Files:**
- Modify: `src/components/views/weekly-view.tsx` (full rewrite)

**Step 1: Rewrite WeeklyView with grid shell**

Replace the entire file with the new structure. Remove all `@dnd-kit` imports (`DndContext`, `DragEndEvent`, `useDraggable`, `useDroppable`, `PointerSensor`, `useSensor`, `useSensors`). Remove `useUpdateEvent` hook. Remove `DayColumn` and `DraggableEvent` components. Remove `handleDragEnd`.

Write the new `WeeklyView` component:
...
git commit -m "feat(weekly): strip DnD, scaffold CSS grid shell with sticky headers"
```

---

### Task 2: Implement HourRow and HourCell

**Files:**
- Modify: `src/components/views/weekly-view.tsx` (append components)

**Step 1: Add HourRow component**

Add below `WeeklyView` in the same file:
...
```bash
git add src/components/views/weekly-view.tsx
git commit -m "feat(weekly): add HourRow and HourCell components"
```

---

### Task 3: Implement EventCard

**Files:**
- Modify: `src/components/views/weekly-view.tsx` (append component)

**Step 1: Add EventCard component**

Same styling as the current `DraggableEvent` but without drag logic:
...
```bash
git add src/components/views/weekly-view.tsx
git commit -m "feat(weekly): add EventCard component with existing styling"
```
...
Check in browser:
- [ ] 24 hour rows visible with time labels in left gutter
- [ ] 7 day columns with sticky headers
- [ ] Events appear in correct hour slots
- [ ] "+N more" badge shows for overlapping events
- [ ] Today column has accent bar + subtle background
- [ ] Clicking empty cell opens create modal with correct day/hour
- [ ] Clicking event opens edit modal
- [ ] Page auto-scrolls to 7 AM on load
...
```bash
git add src/components/views/weekly-view.tsx
git commit -m "fix(weekly): address visual issues from review"
```
